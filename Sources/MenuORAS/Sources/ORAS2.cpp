...
namespace ORAS
{
	...
	void Pokemon1(MenuEntry *entry)
	{
		...
		if (KBValue == 0)
		{
			...
			std::vector<std::string> DayCares = 
			{
				"Route 117",
				"Battle Resort"
			};
			
			KB.Populate(DayCares, false);
			DayCarePlace = KB.Open();
			
			if (!Controller::IsKeyPressed(Key::B))
			{
				if (DayCarePlace == 0) // Route 117 Day Care
				{
				...
				if (DayCarePlace == 1) // Battle Resort Day Care
				{
					// Checks if there is a Pokemon to delete or not. 0x0 meaning nothing and 0x1 meaning there is a Pokemon
					if (READ32(Address[1], g_Data32) && g_Data32 == 0x0)
						MessageBox("There is no Pokemon to delete!", DialogType::DialogOk).SetClear(ClearScreen::Both)();
					
					if (READ32(Address[1], g_Data32) && g_Data32 >= 0x1)
					{
						// Goes through the entire Pokemon's data length and writes 0x0
						for (u8 i = 0x0; i < 0xE4; i++)
							WRITE32(Address[1] + 0x8 + i, 0x0);
						
						WRITE32(Address[1] + 0x4, 0x0);
						WRITE32(Address[1], 0x0);
						MessageBox("Pokemon has been deleted!", DialogType::DialogOk).SetClear(ClearScreen::Both)();
					}
				}
			}
		...
		if (KBValue == 1)
		{
			...
			if (!Controller::IsKeyPressed(Key::B))
			{
				if (DayCarePlace == 0) // Route 117 Day Care
				{
					...
					if (READ32(Address[0], g_Data32) && g_Data32 >= 0x1)
					{
						if (READ32(Address[2], g_Data32) && g_Data32 == 0x0)
						{
							Process::CopyMemory((u8*) (Address[2]), (u8*) (Address[0] + 0x8), 0xE8);
						...
						else if (READ32(Address[2], g_Data32) != 0x0)
				...
				if (DayCarePlace == 1) // Battle Resort Day Care
				{
					...
					if (READ32(Address[1], g_Data32) && g_Data32 >= 0x1)
					{
						if (READ32(Address[2], g_Data32) && g_Data32 == 0x0)
						{
							Process::CopyMemory((u8*) (Address[2]), (u8*) (Address[1] + 0x8), 0xE8);
	...
	void ComputerAnywhere(MenuEntry *entry)
	{
		if (entry->IsActivated())
			WRITE32(AutoGame(0x3D8748, 0x3D8744), 0xE3A00004);
		
		if (!entry->IsActivated())
			WRITE32(AutoGame(0x3D8748, 0x3D8744), 0xE3A00014);
	}
