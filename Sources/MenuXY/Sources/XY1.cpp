...
namespace XY
{
		...
		if (IsInBattle()) // Ensures that the user is in battle
		{
			if (entry->Hotkeys[0].IsDown())
			{
				if (READ32(Pointer, g_Data32) && g_Data32 != 0x0)
				{
					...
					if (READ8(g_Offset32 + 0x778, g_Data8) && g_Data8 == 0x3)
						WRITE8(g_Offset32 + 0x778, 0x4);
				}
			}
		}
	}
	...
	{
		static u32 Address = 0x8C79D86;
		
		if (READ16(Address, g_Data16) && (g_Data16 & ~0xFFF0) != 0x01)
		{
			READ8(Address, g_Data8);
			g_Data8 &= 0xF0;
			g_Data8 += 0x01;
