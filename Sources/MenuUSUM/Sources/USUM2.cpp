...
namespace USUM
{
    ...
    {
		static u32 Address = AutoGame(0x45C628, 0x45C62C);
		
		static u32 Data[3] = 
		{
			0xE3A01001, 0xE5C011E0, 0xEA007D3A
		};
    ...
    {
		// Original non-ASM address is 0x330127EC
		static u32 Address[2] = 
		{
			0x5BBE40,
			0x4AF604
		...
		static u32 Data[7] = 
		{
			...
			0x11A00004, 0xE12FFF1E, 0x00704650
		};
	...
	void ComputerAnywhere(MenuEntry *entry)
	{
		...
		static u32 Buffer[34] =
		{
			...
			0xEAF70DE3, 0xE92D400F, 0xE24DD010, 0xE3A00000, 
			...
			0xEBF77D15, 0xE3A00002, 0xE28DD010, 0xE8BD800F, 
			0x00669000, 0x006A59C4,  
		};
		...
		if (entry->WasJustActivated()) 
		{
			...
			if (!WRITE32(Address[1], 0xEB08F8F7))
				goto Error;
			
			if (!WRITE32(Address[2], 0xEB08C3AA))
				goto Error;
			
			if (!WRITE32(Address[3], 0x1AF70DE3))
				goto Error;
		}
