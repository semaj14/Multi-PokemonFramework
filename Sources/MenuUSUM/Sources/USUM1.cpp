...
namespace USUM
{
	...
	void OneHundredPercentCatch(MenuEntry *entry)
	{
		...
		if (IsInBattle()) // Ensures that the user is in battle
		{
			...
			WRITE32(Address[0] + 0x20, 0x0070C62C);
			WRITE32(Address[1], 0xEB043C5B);
		}
	}
	
	void TrainerRematch(MenuEntry *entry)
	{
		static u32 Address = 0x4BB03C;
    ...
    void ZMovesWithoutCrystal(MenuEntry *entry)
    {
		...
		static u32 Data[23] = 
		{
			0xE92D407D, 0xE1A00006, 0xEA000000, 0xE92D407D,
			0xE1D510B4, 0xE59D201C, 0xE202530F, 0xE3550203,
			0x18BD807D, 0xE59F300C, 0xEB000004, 0xE59F3008,
			0xEB000002, 0xE8BD807D, 0x30000404, 0x300073EC,
			0xE2834018, 0xE4935004, 0xE1550002, 0x01A01000,
			0xE1530004, 0x1AFFFFFA, 0xE12FFF1E,
		};
	...
	void CatchTrainerPokemon(MenuEntry *entry)
	{
		...
		if (IsInBattle()) // Ensures that the user is in battle
		{
			WRITE32(Address[0], 0xE1A04000);
			WRITE32(Address[0] + 0x04, 0xE92D4003);
			WRITE32(Address[0] + 0x08, 0xE59D0028);
			WRITE32(Address[0] + 0x0C, 0xE59F1014);
			WRITE32(Address[0] + 0x10, 0xE1510000);
			WRITE32(Address[0] + 0x14, 0x05970004);
			...
			WRITE32(Address[0] + 0x28, 0x0070BC2C);
			
			if (entry->IsActivated())
				WRITE32(Address[1], 0xEB057993);
			
			if (!entry->IsActivated())
				WRITE32(Address[1], 0xEBFFEB77);
		}
	}
	...
	std::vector<std::string> ListOfForms(int SpawnSpecies)
	{
		...
		switch (SpawnSpecies) 
		{
			...
			case 800: // Necrozma
				Options = {"Normal", "Dawn", "Dusk", "Ultra"};
				break;
	...
	void GetPokemonSpawner(MenuEntry *entry)
	{
		if (entry->IsActivated())
		{
			...
			KB1.SetCompareCallback([](const void *In, std::string &Error)
			{
				...
				if (Input >= 1 && Input <= 807)
					return (true);
				
				Error = "The value must be between 1-807.";
				return (false);
			});
	...
	void SetPokemonSpawner(MenuEntry *entry)
	{
		...
		{
			0x5BBFC0,
			AutoGame(0x3A86A0, 0x3A86A4),
			AutoGame(0x3A8688, 0x3A868C),
			AutoGame(0x3A873C, 0x3A8740)
		};
		
		static u32 Data[10] =
		{
			0xE92D400E, 0xE59F0014, 0xE3500000, 0x18BD800E, 
			0xE59F000C, 0xEBF90532, 0xE2800001, 0xE8BD800E, 
		...
		static u32 Jump[2] = 
		{
			AutoGame(0xEB084E46, 0xEB084E45),
			0xE1D400B0
		};
